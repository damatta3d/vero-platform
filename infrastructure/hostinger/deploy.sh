...
create_environment() {
  if [[ -f "${ENV_FILE}" ]]; then
    chmod 600 "${ENV_FILE}"
    log "Arquivo .env.production existente preservado."
    return
  fi
...
validate_environment() {
  ...
  for key in "${required[@]}"; do
    grep -q "^${key}=." "${ENV_FILE}" || fail "Variável ausente ou vazia em .env.production: ${key}"
  done

  grep -q '^VERO_DATABASE_URL=.*@postgres:5432/' "${ENV_FILE}" \
    || fail "VERO_DATABASE_URL deve apontar para postgres:5432 dentro do Docker."
}

update_repository() {
  if [[ "${VERO_SKIP_GIT_PULL:-false}" == "true" ]]; then
    log "Atualização Git ignorada por VERO_SKIP_GIT_PULL=true."
    return
  fi

  if [[ -n "$(git -C "${REPO_DIR}" status --porcelain --untracked-files=no)" ]]; then
    fail "Existem alterações versionadas locais. Faça commit/stash antes do deploy."
  fi
...
backup_database() {
  if ! compose ps --status running postgres 2>/dev/null | grep -q postgres; then
    log "Banco ainda não está em execução; backup pré-deploy não é necessário."
    return
  fi

  local backup_dir backup_file
  backup_dir="${REPO_DIR}/.runtime/backups"
  backup_file="${backup_dir}/vero-$(date -u +%Y%m%dT%H%M%SZ).sql.gz"
  mkdir -p "${backup_dir}"
  chmod 700 "${REPO_DIR}/.runtime" "${backup_dir}"
  ...
  if compose exec -T postgres sh -c \
    ...
    rm -f "${backup_file}"
    fail "Não foi possível gerar o backup pré-deploy."
  fi
}
...
wait_for_endpoint() {
  ...
  for ((i = 1; i <= attempts; i++)); do
    if curl --fail --silent --show-error --max-time 5 "http://127.0.0.1${path}" >/dev/null; then
      log "Endpoint aprovado: ${path}"
      return
    fi
    sleep 2
  done
  ...
  fail "Endpoint não respondeu com sucesso: ${path}"
}
...
main() {
  ...
  log "Construindo e publicando VERO."
  compose up -d --build --remove-orphans

  log "Aguardando banco e API."
  wait_for_endpoint /health/live
  wait_for_endpoint /health/ready
  wait_for_endpoint /operacao
  wait_for_endpoint /financeiro
  ...
  printf '[VERO] Operação:  http://%s/operacao\n' "${public_ip:-IP-DO-VPS}"
  printf '[VERO] Financeiro: http://%s/financeiro\n' "${public_ip:-IP-DO-VPS}"
