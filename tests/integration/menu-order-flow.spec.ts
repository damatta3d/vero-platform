...
beforeAll(async () => {
  await prisma.$executeRawUnsafe(
    `INSERT INTO catalog_products (id,"tenantId",name,"salePriceCents","createdAt","updatedAt") VALUES ($1::uuid,$2,$3,$4,NOW(),NOW())`,
    productId,
    tenantId,
    'Parmegiana Teste',
    4290
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO commerce_menus (id,tenant_id,name,slug,published,created_at,updated_at) VALUES ($1::uuid,$2,$3,$4,true,NOW(),NOW())`,
    menuId,
    tenantId,
    'Menu Homologacao',
    menuSlug
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO commerce_menu_categories (id,tenant_id,menu_id,name,sort_order,active,created_at,updated_at) VALUES ($1::uuid,$2,$3::uuid,$4,0,true,NOW(),NOW())`,
    categoryId,
    tenantId,
    menuId,
    'Principais'
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO commerce_menu_items (id,tenant_id,menu_id,category_id,catalog_product_id,display_name,sale_price_cents,available,active,featured,sort_order,created_at,updated_at) VALUES ($1::uuid,$2,$3::uuid,$4::uuid,$5::uuid,$6,$7,true,true,false,0,NOW(),NOW())`,
    menuItemId,
    tenantId,
    menuId,
    categoryId,
...
test('validates price server-side, persists order, tracks securely and reaches kitchen queue', async () => {
  const checkout = new PublicCheckoutController(db);
  const validated = await checkout.validate({
    menuSlug,
    fulfillment: 'PICKUP',
    customer: { name: 'Cliente Teste', phone: '67999999999' },
  ...
  const persisted = await prisma.$queryRawUnsafe<Array<{ totalCents: number; trackingTokenHash: string }>>(
    `SELECT total_cents AS "totalCents",tracking_token_hash AS "trackingTokenHash" FROM commerce_native_orders WHERE id=$1::uuid`,
    created.orderId
  );
  ...
  const status = await publicStatus.status(created.orderId, created.trackingToken);
  expect(status).toMatchObject({ orderId: created.orderId, status: 'RECEIVED', fulfillment: 'PICKUP' });
  ...
  const finalStatus = await publicStatus.status(created.orderId, created.trackingToken);
  expect(finalStatus.status).toBe('PREPARING');
