...
export class KitchenOrderController {
  ...
  async transition(@Headers('authorization') authorization: string | undefined, @Headers('x-tenant-id') tenantId: string | undefined, @Param('orderId') orderId: string, @Body() body: { status?: NativeOrderStatus }) {
    ...
    if (!tenantId || !body.status) throw new BadRequestException('Tenant and order status are required.');
    const rows = await this.db.$queryRawUnsafe<Array<{ status: NativeOrderStatus }>>(`SELECT status FROM commerce_native_orders WHERE id=$1::uuid AND tenant_id=$2`, orderId, tenantId);
    const current = rows[0]?.status;
    if (!current) throw new BadRequestException('ORDER_NOT_FOUND');
    try { assertOrderTransition(current, body.status); } catch (error) { throw new BadRequestException(error instanceof Error ? error.message : 'Invalid order transition.'); }
    const changed = await this.db.$executeRawUnsafe(`UPDATE commerce_native_orders SET status=$1,updated_at=NOW() WHERE id=$2::uuid AND tenant_id=$3 AND status=$4`, body.status, orderId, tenantId, current);
    if (changed !== 1) throw new BadRequestException('ORDER_CONCURRENTLY_CHANGED');
    await this.db.$executeRawUnsafe(`INSERT INTO commerce_native_order_status_history (id,order_id,from_status,to_status,occurred_at) VALUES ($1::uuid,$2::uuid,$3,$4,NOW())`, randomUUID(), orderId, current, body.status);
