  ...
  <body>
    ...
    <main>
      ...
      <section id="checkout" class="hidden">
        ...
        <label>E-mail (opcional)<input id="customer-email" type="email" autocomplete="email"></label>
        ...
        <div id="address" class="hidden">
          <label>CEP<input id="postal-code" inputmode="numeric" maxlength="9"></label>
          <label>Rua<input id="street"></label>
          <label>Número<input id="number"></label>
          <label>Bairro<input id="district"></label>
          <label>Cidade<input id="city"></label>
          <label>UF<input id="state-code" maxlength="2" autocapitalize="characters"></label>
          <label>Complemento<input id="complement"></label>
          <label>Referência<input id="reference"></label>
          <p id="delivery-quote-status" class="coupon-message" aria-live="polite"></p>
        </div>
        <div class="totals checkout-totals">
          <span>Subtotal</span><span id="checkout-subtotal">R$ 0,00</span>
          <span id="checkout-discount-label" class="hidden">Desconto</span><span id="checkout-discount" class="hidden"></span>
          <span>Entrega <small id="checkout-distance"></small></span><span id="checkout-delivery">R$ 0,00</span>
          <strong>Total</strong><strong id="checkout-total">R$ 0,00</strong>
        </div>
...
const javascript = `
let menu=null,cart=[],appliedCoupon=null,couponValidation=0,lastPixCode='',deliveryQuote=null,deliveryQuoteRequest=0,deliveryQuoteTimer=null;
...
function configure(){const c=menu.checkout||{};$('fulfillment').innerHTML=(c.pickupEnabled?'<option value="PICKUP">Retirar no restaurante</option>':'')+(c.deliveryEnabled?'<option value="DELIVERY">Receber por entrega</option>':'');$('method').innerHTML=(c.paymentOnDeliveryEnabled?'<option value="PAY_ON_DELIVERY">Pagar ao receber</option>':'')+(c.pixEnabled?'<option value="PIX">PIX</option>':'');syncAddress();syncPayment()}
function renderCheckoutQuote(){const itemsTotal=deliveryQuote?.itemsTotalCents??subtotal(),discount=deliveryQuote?.discountCents??Math.min(itemsTotal,appliedCoupon?.discountCents||0),fee=$('fulfillment').value==='DELIVERY'?(deliveryQuote?.deliveryFeeCents||0):0,total=itemsTotal-discount+fee;$('checkout-subtotal').textContent=money(itemsTotal);$('checkout-discount').textContent='− '+money(discount);$('checkout-discount').classList.toggle('hidden',!discount);$('checkout-discount-label').classList.toggle('hidden',!discount);$('checkout-delivery').textContent=money(fee);$('checkout-distance').textContent=deliveryQuote?.distanceMeters!=null?'— '+(deliveryQuote.distanceMeters/1000).toLocaleString('pt-BR',{maximumFractionDigits:1})+' km':'';$('checkout-total').textContent=money(total);$('finish').disabled=!$('fulfillment').options.length||!$('method').options.length||($('fulfillment').value==='DELIVERY'&&!deliveryQuote)}
function syncAddress(){const delivery=$('fulfillment').value==='DELIVERY';$('address').classList.toggle('hidden',!delivery);deliveryQuote=delivery?null:{itemsTotalCents:subtotal(),discountCents:Math.min(subtotal(),appliedCoupon?.discountCents||0),deliveryFeeCents:0,totalCents:subtotal()-Math.min(subtotal(),appliedCoupon?.discountCents||0)};$('delivery-quote-status').textContent='';renderCheckoutQuote();if(delivery)scheduleDeliveryQuote()}
function syncPayment(){$('customer-email').required=false}
...
function deliveryAddress(){if($('fulfillment').value!=='DELIVERY')return null;const address={postalCode:$('postal-code').value.trim(),street:$('street').value.trim(),number:$('number').value.trim(),district:$('district').value.trim(),city:$('city').value.trim(),stateCode:$('state-code').value.trim().toUpperCase(),complement:$('complement').value.trim()||undefined,reference:$('reference').value.trim()||undefined};if(!address.postalCode||!address.street||!address.number||!address.district||!address.city||!address.stateCode)throw new Error('Informe CEP, rua, número, bairro, cidade e UF.');return address}
function scheduleDeliveryQuote(){if($('checkout').classList.contains?.('hidden')||$('fulfillment').value!=='DELIVERY')return;clearTimeout(deliveryQuoteTimer);deliveryQuoteTimer=setTimeout(()=>void quoteDelivery(),450)}
async function quoteDelivery(){if($('fulfillment').value!=='DELIVERY'){syncAddress();return true}let address;try{address=deliveryAddress()}catch{deliveryQuote=null;renderCheckoutQuote();return false}const request=++deliveryQuoteRequest;$('delivery-quote-status').textContent='Calculando entrega…';$('delivery-quote-status').className='coupon-message';$('finish').disabled=true;try{const response=await fetch('/v1/checkout/delivery-quote',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({menuSlug:slug,couponCode:appliedCoupon?.code,address,items:orderItems()})});if(!response.ok)throw new Error(await msg(response,'Não foi possível calcular a entrega.'));const quote=await response.json();if(request!==deliveryQuoteRequest)return false;deliveryQuote=quote;$('delivery-quote-status').textContent=quote.normalizedAddress?'Endereço validado: '+quote.normalizedAddress:'Entrega calculada.';$('delivery-quote-status').className='coupon-message success';renderCheckoutQuote();return true}catch(error){if(request!==deliveryQuoteRequest)return false;deliveryQuote=null;$('delivery-quote-status').textContent=error instanceof Error?error.message:'Não foi possível calcular a entrega.';$('delivery-quote-status').className='coupon-message error';renderCheckoutQuote();return false}}
function clearCoupon(message=''){appliedCoupon=null;couponValidation+=1;$('coupon-message').textContent=message;$('coupon-message').className='coupon-message';$('remove-coupon').classList.add('hidden');renderTotals(subtotal());deliveryQuote=null;renderCheckoutQuote();scheduleDeliveryQuote()}
async function validateCoupon(silent=false){const code=$('coupon-code').value.trim();if(!code){clearCoupon();return false}const validation=++couponValidation;$('apply-coupon').disabled=true;if(!silent){$('coupon-message').textContent='Validando cupom…';$('coupon-message').className='coupon-message'}try{const response=await fetch('/v1/checkout/price',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({menuSlug:slug,couponCode:code,items:orderItems()})});if(!response.ok)throw new Error(await msg(response,'Cupom inválido.'));const pricing=await response.json();if(validation!==couponValidation)return false;if(!pricing.coupon)throw new Error('Cupom inválido.');appliedCoupon={code:pricing.coupon.code,discountCents:pricing.discountCents};$('coupon-code').value=pricing.coupon.code;$('coupon-message').textContent='Cupom '+pricing.coupon.code+' aplicado.';$('coupon-message').className='coupon-message success';$('remove-coupon').classList.remove('hidden');renderTotals(pricing.itemsTotalCents,pricing.discountCents);deliveryQuote=null;scheduleDeliveryQuote();return true}catch(error){if(validation!==couponValidation)return false;appliedCoupon=null;$('coupon-message').textContent=error instanceof Error?error.message:'Cupom inválido.';$('coupon-message').className='coupon-message error';$('remove-coupon').classList.add('hidden');renderTotals(subtotal());deliveryQuote=null;scheduleDeliveryQuote();return false}finally{if(validation===couponValidation)$('apply-coupon').disabled=false}}
function trackingHref(order){return '/pedido/'+encodeURIComponent(order.orderId)+'?token='+encodeURIComponent(order.trackingToken)}
function renderPaid(order){const href=trackingHref(order);$('success').innerHTML='<h2>'+(order.orderNumber?'Pedido #'+esc(order.orderNumber):'Pedido recebido')+'</h2><section class="pix-payment paid"><h3>Pagamento confirmado</h3><p>Seu PIX foi confirmado. O pedido já está seguindo para o restaurante.</p></section><a href="'+href+'">Acompanhar pedido</a>';if(typeof globalThis.setTimeout==='function'&&globalThis.location&&typeof globalThis.location.assign==='function'){globalThis.setTimeout(()=>globalThis.location.assign(href),1500)}}
function pollPayment(order){if(typeof globalThis.setTimeout!=='function')return;const url='/v1/orders/'+encodeURIComponent(order.orderId)+'/status?token='+encodeURIComponent(order.trackingToken);let attempts=0;const check=async()=>{attempts++;try{const response=await fetch(url,{cache:'no-store'});if(response.ok){const status=await response.json();if(status.paymentStatus==='PAID'){renderPaid(order);return}if(['FAILED','CANCELLED','REFUNDED','CHARGED_BACK'].includes(status.paymentStatus)){return}}}catch{}if(attempts<240)globalThis.setTimeout(check,2500)};globalThis.setTimeout(check,1800)}
async function finish(){const button=$('finish'),customer={name:$('customer-name').value.trim(),phone:$('customer-phone').value.trim(),email:$('customer-email').value.trim()||undefined};button.disabled=true;try{if(!menu?.checkout?.canAcceptOrders)throw new Error(menu?.checkout?.statusMessage||'A loja não está aceitando pedidos agora.');if(!customer.name||!customer.phone)throw new Error('Informe nome e telefone.');const fulfillment=$('fulfillment').value,method=$('method').value,address=deliveryAddress(),orderNote=$('order-note').value.trim()||undefined,orderLines=orderItems(),idempotencyKey=secureKey(),couponCode=appliedCoupon?.code;if(fulfillment==='DELIVERY'&&!(await quoteDelivery()))throw new Error('Calcule uma entrega válida antes de finalizar.');const draft={menuSlug:slug,couponCode,customer,fulfillment,address,orderNote,items:orderLines};const validation=await fetch('/v1/checkout/validate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(draft)});if(!validation.ok)throw new Error(await msg(validation,'Revise o pedido.'));const paymentResponse=await fetch('/v1/payments',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...draft,checkoutId:idempotencyKey,method})});if(!paymentResponse.ok)throw new Error(await msg(paymentResponse,'Pagamento indisponível.'));const payment=await paymentResponse.json();if(['FAILED','CANCELLED','REFUNDED','CHARGED_BACK'].includes(payment.status))throw new Error('O pagamento não foi aprovado. Tente novamente.');const orderResponse=await fetch('/v1/orders/native',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...draft,idempotencyKey,payment:{method:payment.method,paymentId:payment.paymentId}})});if(!orderResponse.ok)throw new Error(await msg(orderResponse,'Não foi possível criar o pedido.'));const order=await orderResponse.json();cart=[];$('coupon-code').value='';clearCoupon();renderCart();$('checkout').classList.add('hidden');lastPixCode=payment.pixCopyPaste||'';const pix=payment.method==='PIX'?'<section class="pix-payment"><h3>Aguardando pagamento</h3><p>Valor: <strong>'+money(payment.amountCents)+'</strong></p>'+(payment.qrCodeUrl?'<img src="'+esc(payment.qrCodeUrl)+'" alt="QR Code PIX">':'')+(lastPixCode?'<label>PIX copia e cola<input class="pix-code" readonly value="'+esc(lastPixCode)+'"></label><button type="button" data-copy-pix>Copiar código PIX</button>':'')+'<p>Aguardando confirmação do pagamento…</p></section>':'';$('success').innerHTML='<h2>'+(order.orderNumber?'Pedido #'+esc(order.orderNumber):'Pedido recebido')+'</h2><p>Pedido recebido com sucesso.</p>'+pix+'<a href="'+trackingHref(order)+'">Acompanhar pedido</a>';$('success').classList.remove('hidden');if(payment.method==='PIX')pollPayment(order)}catch(error){$('checkout-message').textContent=error instanceof Error?error.message:'Não foi possível finalizar o pedido.';$('checkout-message').className='error'}finally{renderCheckoutQuote()}}
...
;['postal-code','street','number','district','city','state-code','complement','reference'].forEach(id=>$(id).addEventListener('input',()=>{deliveryQuote=null;renderCheckoutQuote();scheduleDeliveryQuote()}))
