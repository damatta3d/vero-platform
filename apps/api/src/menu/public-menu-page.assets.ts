...
const javascript = `
let menu=null,cart=[],appliedCoupon=null,couponValidation=0;
...
async function finish(){const button=$('finish'),customer={name:$('customer-name').value.trim(),phone:$('customer-phone').value.trim()};button.disabled=true;try{if(!menu?.checkout?.canAcceptOrders)throw new Error(menu?.checkout?.statusMessage||'A loja não está aceitando pedidos agora.');if(!customer.name||!customer.phone)throw new Error('Informe nome e telefone.');const fulfillment=$('fulfillment').value,method=$('method').value,address=deliveryAddress(),orderNote=$('order-note').value.trim()||undefined,orderLines=orderItems(),idempotencyKey=secureKey(),couponCode=appliedCoupon?.code;const validation=await fetch('/v1/checkout/validate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({menuSlug:slug,couponCode,customer,fulfillment,address,orderNote,items:orderLines})});if(!validation.ok)throw new Error(await msg(validation,'Revise o pedido.'));const paymentResponse=await fetch('/v1/payments',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({checkoutId:idempotencyKey,menuSlug:slug,couponCode,method,customerName:customer.name,customerPhone:customer.phone,items:orderLines.map(x=>({menuItemId:x.menuItemId,quantity:x.quantity}))})});if(!paymentResponse.ok)throw new Error(await msg(paymentResponse,'Pagamento indisponível.'));const payment=await paymentResponse.json();const orderResponse=await fetch('/v1/orders/native',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({idempotencyKey,menuSlug:slug,couponCode,customer,fulfillment,address,orderNote,items:orderLines,payment:{method:payment.method,status:payment.status,paymentId:payment.paymentId}})});if(!orderResponse.ok)throw new Error(await msg(orderResponse,'Não foi possível criar o pedido.'));const order=await orderResponse.json();cart=[];$('coupon-code').value='';clearCoupon();renderCart();$('checkout').classList.add('hidden');$('success').innerHTML='<h2>'+(order.orderNumber?'Pedido #'+esc(order.orderNumber):'Pedido recebido')+'</h2><p>Pedido recebido com sucesso.</p><a href="/pedido/'+encodeURIComponent(order.orderId)+'?token='+encodeURIComponent(order.trackingToken)+'">Acompanhar pedido</a>';$('success').classList.remove('hidden')}catch(error){$('checkout-message').textContent=error instanceof Error?error.message:'Não foi possível finalizar o pedido.';$('checkout-message').className='error'}finally{button.disabled=false}}
