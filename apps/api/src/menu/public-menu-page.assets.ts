...
<body>
  ...
  <main class="layout">
    ...
    <aside class="cart panel" aria-labelledby="cart-title">
      ...
      <div class="coupon-entry"><input id="coupon-code" maxlength="64" autocomplete="off" placeholder="Cupom de desconto" aria-label="Cupom de desconto"><button id="apply-coupon" class="link-button" type="button">Aplicar</button></div>
      <p id="coupon-message" class="coupon-message" role="status"></p>
      <div class="cart-total"><span>Subtotal</span><strong id="cart-subtotal">R$ 0,00</strong></div>
      <div id="cart-discount-row" class="cart-total discount hidden"><span>Desconto</span><strong id="cart-discount">− R$ 0,00</strong></div>
      <div class="cart-total grand-total"><span>Total</span><strong id="cart-total">R$ 0,00</strong></div>
...
async function validateCoupon(silent=false){const code=byId('coupon-code').value.trim();const message=byId('coupon-message');const button=byId('apply-coupon');const validation=++couponValidation;if(!code){appliedCoupon=null;message.textContent='';message.className='coupon-message';renderTotals(lines().reduce((sum,{line,item})=>sum+item.priceCents*line.quantity,0));return;}button.disabled=true;if(!silent){message.textContent='Validando cupom…';message.className='coupon-message';}try{const response=await fetch('/v1/checkout/price',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({menuSlug:slug,couponCode:code,items:checkoutLines()})});if(!response.ok)throw new Error(await errorMessage(response,'Cupom inválido.'));const pricing=await response.json();if(validation!==couponValidation)return;appliedCoupon={code:pricing.coupon.code,discountCents:pricing.discountCents};byId('coupon-code').value=pricing.coupon.code;message.textContent='Cupom '+pricing.coupon.code+' aplicado.';message.className='coupon-message success';renderTotals(pricing.itemsTotalCents,pricing.discountCents);}catch(error){if(validation!==couponValidation)return;appliedCoupon=null;message.textContent=error instanceof Error?error.message:'Cupom inválido.';message.className='coupon-message error';renderTotals(lines().reduce((sum,{line,item})=>sum+item.priceCents*line.quantity,0));}finally{if(validation===couponValidation)button.disabled=false;}}
function createIdempotencyKey(){const randomBytes=new Uint8Array(32);if(!globalThis.crypto||typeof globalThis.crypto.getRandomValues!=='function')throw new Error('Seu navegador não oferece geração segura para finalizar o pedido.');globalThis.crypto.getRandomValues(randomBytes);return Array.from(randomBytes,(value)=>value.toString(16).padStart(2,'0')).join('');}
...
async function finish(){const button=byId('finish');const message=byId('checkout-message');button.disabled=true;try{const customer={name:byId('customer-name').value.trim(),phone:byId('customer-phone').value.trim()};if(!customer.name||!customer.phone)throw new Error('Informe seu nome e telefone.');message.textContent='Validando seu pedido…';message.className='message';const idempotencyKey=createIdempotencyKey();const orderLines=checkoutLines();const couponCode=appliedCoupon?.code;const checkout=await fetch('/v1/checkout/validate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({menuSlug:slug,couponCode,customer,fulfillment:'PICKUP',address:null,items:orderLines})});if(!checkout.ok)throw new Error(await errorMessage(checkout,'Revise os itens do pedido.'));const paymentResponse=await fetch('/v1/payments',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({checkoutId:idempotencyKey,menuSlug:slug,couponCode,method:'PAY_ON_DELIVERY',customerName:customer.name,customerPhone:customer.phone,items:orderLines.map((line)=>({menuItemId:line.menuItemId,quantity:line.quantity}))})});if(!paymentResponse.ok)throw new Error(await errorMessage(paymentResponse,'Não foi possível preparar o pagamento.'));const payment=await paymentResponse.json();const orderResponse=await fetch('/v1/orders/native',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({idempotencyKey,menuSlug:slug,couponCode,customer,fulfillment:'PICKUP',items:orderLines,payment:{method:payment.method,status:payment.status,paymentId:payment.paymentId}})});if(!orderResponse.ok)throw new Error(await errorMessage(orderResponse,'Não foi possível criar o pedido.'));const order=await orderResponse.json();cart=[];appliedCoupon=null;byId('coupon-code').value='';persist();renderCart();byId('checkout').classList.add('hidden');byId('success').innerHTML='<p class="eyebrow">PEDIDO RECEBIDO</p><h2>Pedido #'+escapeHtml(order.orderId.slice(0,8))+'</h2><p>Acompanhe o preparo sem expor seus dados.</p><a href="/pedido/'+encodeURIComponent(order.orderId)+'?token='+encodeURIComponent(order.trackingToken)+'">Acompanhar pedido</a>';byId('success').classList.remove('hidden');byId('success').scrollIntoView({behavior:'smooth'});}catch(error){message.textContent=error instanceof Error?error.message:'Não foi possível finalizar o pedido.';message.className='message error';}finally{button.disabled=false;}}
...
byId('apply-coupon').addEventListener('click',()=>validateCoupon());
byId('coupon-code').addEventListener('input',()=>{appliedCoupon=null;couponValidation+=1;byId('coupon-message').textContent='';byId('coupon-message').className='coupon-message';renderTotals(lines().reduce((sum,{line,item})=>sum+item.priceCents*line.quantity,0));});
