...
export class MenuAdminController {
  ...
  async update(
    ...
    const tenant = await this.authorize(authorization, tenantHeader);
    await this.database.$executeRawUnsafe(
      `UPDATE commerce_menus SET name=COALESCE($3,name), slug=COALESCE($4,slug), description=COALESCE($5,description),
       logo_url=COALESCE($6,logo_url), cover_url=COALESCE($7,cover_url), published=COALESCE($8,published), updated_at=NOW()
       WHERE tenant_id=$1 AND id=$2::uuid`,
      tenant, menuId, body.name ?? null, body.slug ?? null, body.description ?? null, body.logoUrl ?? null, body.coverUrl ?? null, body.published ?? null
    );
  ...
  async updateCategory(
    ...
    await this.database.$executeRawUnsafe(
      `UPDATE commerce_menu_categories SET name=COALESCE($3,name), description=COALESCE($4,description),
       sort_order=COALESCE($5,sort_order), active=COALESCE($6,active), updated_at=NOW() WHERE tenant_id=$1 AND id=$2::uuid`,
  ...
  async addItem(
    ...
    await this.database.$executeRawUnsafe(
      ...
      id, tenant, menuId, body.categoryId, body.catalogProductId, body.displayName ?? null, body.description ?? null,
      body.imageUrl ?? null, body.salePriceCents ?? null, body.sortOrder ?? 0, body.featured ?? false
    );
  ...
  async updateItem(
    ...
    await this.database.$executeRawUnsafe(
      `UPDATE commerce_menu_items SET category_id=COALESCE($3::uuid,category_id), display_name=COALESCE($4,display_name),
       ...
       sort_order=COALESCE($8,sort_order), active=COALESCE($9,active), available=COALESCE($10,available), featured=COALESCE($11,featured),
       updated_at=NOW() WHERE tenant_id=$1 AND id=$2::uuid`,
      tenant, itemId, body.categoryId ?? null, body.displayName ?? null, body.description ?? null, body.imageUrl ?? null,
      body.salePriceCents ?? null, body.sortOrder ?? null, body.active ?? null, body.available ?? null, body.featured ?? null
    );
