...
export class PaymentWebhookController {
  ...
  async receive(
    ...
    if (!secret || !accessToken || !signature || !requestId || !dataId)
    ...
    const changed = await this.db.$executeRawUnsafe(
      `UPDATE commerce_native_orders SET payment_status=$1,updated_at=NOW() WHERE provider_payment_id=$2 AND payment_status<>$1`,
      paymentStatus,
      dataId
    );
    return {
