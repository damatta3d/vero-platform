...
type PaymentInput = { menuSlug: string; method: PaymentMethod; customerName: string; customerPhone: string; items: Array<{ menuItemId: string; quantity: number }> };
...
export class PaymentController {
  ...
  async create(@Body() request: PaymentInput) {
    if (!request.menuSlug?.trim() || !request.customerName?.trim() || !request.customerPhone?.trim() || !request.items?.length) throw new BadRequestException('Missing payment data.');
    if (request.items.some((item) => !Number.isInteger(item.quantity) || item.quantity <= 0)) throw new BadRequestException('Invalid payment items.');

    const ids = request.items.map((item) => item.menuItemId);
    const rows = await this.db.$queryRawUnsafe<Row[]>(`SELECT i.id AS "menuItemId",COALESCE(i.sale_price_cents,p."salePriceCents") AS "priceCents",i.available FROM commerce_menu_items i JOIN commerce_menus m ON m.tenant_id=i.tenant_id AND m.id=i.menu_id JOIN catalog_products p ON p."tenantId"=i.tenant_id AND p.id=i.catalog_product_id WHERE m.slug=$1 AND m.published=true AND i.active=true AND i.id=ANY($2::uuid[])`, request.menuSlug, ids);
    ...
    const amountCents = request.items.reduce((sum, item) => sum + (prices.get(item.menuItemId) ?? 0) * item.quantity, 0);
    if (amountCents <= 0) throw new BadRequestException('Invalid payment amount.');

    const trusted = { menuSlug: request.menuSlug, method: request.method, amountCents, customerName: request.customerName.trim(), customerPhone: request.customerPhone.trim() };
    ...
    return new MercadoPagoPaymentGateway(accessToken, process.env.MERCADO_PAGO_NOTIFICATION_URL?.trim()).createPayment(trusted);
  }
}
