...
export class NativeOrderController {
  ...
  async create(@Body() request: Request) {
    ...
    try {
      await this.db.$executeRawUnsafe(`INSERT INTO commerce_native_orders (id,tenant_id,menu_slug,provider,customer_name,customer_phone,fulfillment,items_total_cents,delivery_fee_cents,total_cents,payment_method,payment_status,provider_payment_id,status,created_at,updated_at) VALUES ($1::uuid,$2,$3,'VERO_NATIVE',$4,$5,$6,$7,0,$7,$8,$9,$10,'RECEIVED',$11::timestamptz,$11::timestamptz)`, orderId, tenantId, request.menuSlug, request.customer.name.trim(), request.customer.phone.trim(), request.fulfillment, itemsTotalCents, request.payment.method, request.payment.status, request.payment.paymentId || null, createdAt);
    ...
    return { orderId, provider: 'VERO_NATIVE', menuSlug: request.menuSlug, fulfillment: request.fulfillment, items, itemsTotalCents, deliveryFeeCents: 0, totalCents: itemsTotalCents, paymentMethod: request.payment.method, paymentStatus: request.payment.status, status: 'RECEIVED', createdAt };
  }
}
