...
export class NativeOrderController {
  ...
  async create(@Body() request: Request) {
    if (!request.menuSlug?.trim() || !request.customer?.name?.trim() || !request.customer?.phone?.trim() || !request.items?.length) {
      throw new BadRequestException('Invalid native order.');
    }
    if (request.payment.method === 'PIX' && request.payment.status !== 'PAID') {
      throw new BadRequestException('PIX payment must be confirmed before order creation.');
    }
    ...
    const items = request.items.map((line) => {
      ...
      return { menuItemId: item.menuItemId, name: item.name, quantity: line.quantity, unitPriceCents: item.priceCents,
        totalCents: item.priceCents * line.quantity, note: line.note?.trim() || null };
    });
    const itemsTotalCents = items.reduce((sum, item) => sum + item.totalCents, 0);
    const orderId = randomUUID();

    return { orderId, externalOrderId: orderId, provider: 'VERO_NATIVE', menuSlug: request.menuSlug,
      customerName: request.customer.name.trim(), customerPhone: request.customer.phone.trim(), fulfillment: request.fulfillment,
      items, itemsTotalCents, deliveryFeeCents: 0, totalCents: itemsTotalCents,
      paymentMethod: request.payment.method, paymentStatus: request.payment.status,
      providerPaymentId: request.payment.paymentId || null, status: 'RECEIVED', createdAt: new Date().toISOString() };
  }
}
