...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,system-ui,sans-serif}
    ...
    table{width:100%;border-collapse:collapse;font-size:14px}th,td{text-align:left;padding:10px;border-bottom:1px solid var(--line);vertical-align:top}th{color:var(--muted);font-size:12px;text-transform:uppercase}.income{color:var(--green);font-weight:700}.outflow{color:var(--red);font-weight:700}.status{font-size:12px;padding:3px 8px;border-radius:99px;background:#eee}.empty{color:var(--muted);padding:16px 0}.message{margin-top:12px;font-weight:700}.error{color:var(--red)}.success{color:var(--green)}
  ...
  <section class="grid content">
    <article class="card">
      ...
      <form id="entryForm" class="grid form">
        <div><label>Tipo</label><select name="type"><option value="INCOME">Receita</option><option value="EXPENSE">Despesa</option><option value="PURCHASE">Compra</option><option value="WITHDRAWAL">Retirada</option><option value="ADJUSTMENT">Ajuste</option></select></div>
...
async function load(){const message=document.querySelector('#message');message.textContent='';try{const [summaryRes,listRes]=await Promise.all([fetch('/v1/operations/summary'+range(),{headers:headers()}),fetch('/v1/operations'+range()+'&limit=200',{headers:headers()})]);if(!summaryRes.ok||!listRes.ok)throw new Error('Confira a chave de acesso e o período.');const summary=await summaryRes.json();const list=await listRes.json();document.querySelector('#income').textContent=money(summary.incomeCents);document.querySelector('#outflow').textContent=money(summary.outflowCents);document.querySelector('#pending').textContent=money(summary.pendingCents);document.querySelector('#orders').textContent=summary.orderCount;document.querySelector('#balance').textContent=money(summary.balanceCents);const body=document.querySelector('#entries');body.innerHTML=list.length?list.map((e)=>{const out=e.type!=='INCOME';return '<tr><td>'+new Date(e.occurredAt).toLocaleDateString('pt-BR')+'</td><td>'+e.type+'</td><td><strong>'+e.description+'</strong><br><small>'+e.category+(e.channel?' · '+e.channel:'')+'</small></td><td><span class="status">'+e.status+'</span></td><td class="'+(out?'outflow':'income')+'">'+(out?'- ':'+ ')+money(e.amountCents)+'</td></tr>'}).join(''):'<tr><td colspan="5" class="empty">Nenhum lançamento no período.</td></tr>'; }catch(error){message.className='message error';message.textContent=error instanceof Error?error.message:'Erro ao carregar dados.'}}
