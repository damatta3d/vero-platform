...
function decimalNumber(value){const raw=String(value??'').trim();const normalized=raw.includes(',')?raw.replace(/\\./g,'').replace(',','.'):raw;const number=Number(normalized);return Number.isFinite(number)?number:NaN;}
...
function resetIngredientForm(){editingIngredientId=null;const form=$('ingredientForm');if(form)form.reset();const submit=$('ingredientSubmit');const cancel=$('ingredientCancel');const quantity=$('ingredientQuantity');if(submit)submit.textContent='Salvar item';if(cancel)cancel.classList.add('hidden');if(quantity)quantity.placeholder='Ex.: 1,200 kg ou 100 unidades';}
function renderIngredientMaintenance(){const list=$('ingredientList');if(!list)return;list.className=state.ingredients.length?'list':'list empty';list.innerHTML=state.ingredients.length?state.ingredients.map(i=>'<div class="row"><div><strong>'+escapeHtml(i.name)+'</strong><small>'+(i.kind==='PACKAGING'?'Embalagem':'Insumo')+' · '+formatQuantity(i.packageQuantityMicros,i.unit)+' · '+money(i.packageCostCents)+'</small></div><div class="item-actions"><button type="button" class="secondary" data-edit-ingredient="'+i.id+'">Alterar</button><button type="button" class="danger" data-delete-ingredient="'+i.id+'">Excluir</button></div></div>').join(''):'Nenhum item cadastrado.';}`
  )
  .replace(
    ...
    "async function api(path,options={}){const response=await fetch(path,{...options,headers:{...headers(),...(options.headers||{})}});if(!response.ok){let detail='';try{const payload=await response.json();detail=Array.isArray(payload.message)?payload.message.join(', '):(payload.message||'');}catch{}throw new Error(response.status===401?'Acesso negado. Confira a chave e a empresa.':detail||'Não foi possível concluir a operação.');}return response.status===204?null:response.json();}"
  )
  .replace(
...
const unitLabels={KILOGRAM:'kg',GRAM:'g',LITER:'L',MILLILITER:'mL',UNIT:'unid.'};
...
function formatMoneyInput(input){if(!input.value.trim())return;input.value=(moneyToCents(input.value)/100).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2});}`
  ...
  .replace(
    ...
    "function formatQuantity(micros,unit){const quantity=micros/1000000;const digits=unit==='UNIT'?0:3;return new Intl.NumberFormat('pt-BR',{minimumFractionDigits:digits,maximumFractionDigits:digits}).format(quantity)+' '+unitLabel(unit);}"
  )
  .replace(
  ...
  .replace(
    "function formatQuantity(micros,unit){return new Intl.NumberFormat('pt-BR',{maximumFractionDigits:3}).format(micros/1000000)+' '+unitLabel(unit);}",
    "function formatQuantity(micros,unit){const quantity=micros/1000000;const digits=unit==='UNIT'?0:3;return new Intl.NumberFormat('pt-BR',{minimumFractionDigits:digits,maximumFractionDigits:digits}).format(quantity)+' '+unitLabel(unit);}"
  )
  .replace(
    ...
    "async function refresh(){if(!$('apiKey').value)return;try{[state.ingredients,state.products,state.positions,state.production,state.productionSummary,state.sales,state.salesSummary]=await Promise.all([api('/v1/catalog/ingredients'),api('/v1/catalog/products'),api('/v1/inventory/positions'),api('/v1/production?limit=20'),api('/v1/production/summary'),api('/v1/sales?limit=20'),api('/v1/sales/summary')]);render();renderIngredientMaintenance();}catch(error){toast(error.message,true);}}"
  )
  .replace(
