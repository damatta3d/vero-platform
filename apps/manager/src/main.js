...
async function loadSelectedMenu() {
  const menu = state.menus.find((candidate) => candidate.id === state.selectedMenuId);
  if (!menu) return;
  try {
    state.publicMenu = await api(`/v1/menu/${encodeURIComponent(menu.slug)}`);
    renderCatalogDetail(menu, state.publicMenu);
  } catch (error) {
    if (!menu.published) {
      renderCatalogDetail(menu, null);
      return;
    }
    catalogDetail.innerHTML = `<p class="muted">Erro ao carregar detalhes: ${escapeHtml(error.message)}</p>`;
  }
}
function renderCatalogDetail(menu, detail) {
  const categories = detail?.categories || [];
  catalogDetail.innerHTML = `<div class="catalog-title"><div><p class="eyebrow">MENU</p><h2>${escapeHtml(menu.name)}</h2><p class="muted">${escapeHtml(menu.description || 'Sem descrição')}</p></div><button id="toggle-publish" class="primary-button" type="button">${menu.published ? 'Despublicar' : 'Publicar'}</button></div><div class="catalog-categories">${categories.length ? categories.map((category) => `<section class="catalog-category"><div class="category-heading"><h3>${escapeHtml(category.name)}</h3><span>${category.items.length} itens</span></div>${category.items.map((item) => `<article class="catalog-item"><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.description || '')}</small></div><div class="catalog-item-actions"><strong>${money(item.priceCents)}</strong><button type="button" data-item-id="${escapeHtml(item.id)}" data-available="${item.available}">${item.available ? 'Pausar' : 'Ativar'}</button></div></article>`).join('')}</section>`).join('') : `<div class="empty-catalog"><p class="muted">${menu.published ? 'Nenhuma categoria ativa encontrada.' : 'Publique o menu para visualizar sua estrutura pública. A criação e edição avançada entram no próximo incremento.'}</p></div>`}</div>`;
  document
