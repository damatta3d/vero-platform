...
describe('AuthenticatedPrincipal', () => {
  it.each(['human', 'service'] as const)('creates an immutable %s principal', (type) => {
    const principal = createTrustedPrincipal('trusted-authority', 'subject-1', type);
  ...
  it('qualifies subject identity by authority', () => {
    const first = createTrustedPrincipal('authority-a', 'same-subject', 'human');
    const second = createTrustedPrincipal('authority-b', 'same-subject', 'human');
  ...
  it.each([['', 'subject'], ['authority', '']])(
    'rejects incomplete trusted identity',
    (authority, subject) => {
